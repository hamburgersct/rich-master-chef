...
-- * Handlers that catch & process player behaviors

kitchenPlaceA :: PlayerState -> IO (Result Kitchen)
kitchenPlaceA ps =
  if (pBudget (psChef ps)) == 0
    then return Retry
...
-- * Handlers that update player state (budget, earning, etc.) after operations
...
serveUpdatePS ps =
  ps
    { psBudget = budgetDown (psBudget ps) 5,
...
restUpdatePS ps =
  ps
    { psBudget = budgetUp (psBudget ps) 100,
